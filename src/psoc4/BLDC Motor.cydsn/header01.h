...
struct BLDC_4_00
{
    ...
    uint8 inttrig;
    uint8 refstate,comtable[6], seqCW[6], seqCCW[6];
...
void setspeedA(struct BLDC_4_00 *p, float sped){
    ...
    PWM_A_WriteCompare(((temp < 0)?0:temp)*4800);
}
...
void setspeedB(struct BLDC_4_00 *p, float sped){
    ...
    float speed = sped*48;
    float temp =((fabs(speed))/p->maxspeed);
    PWM_B_WriteCompare(((temp < 0)?0:temp)*4800);
}
...
void setspeedC(struct BLDC_4_00 *p, float sped){
    ...
    float speed = 4800 - (sped*48);
    float temp =((fabs(speed))/p->maxspeed);
    PWM_C_WriteCompare(((temp < 0)?0:temp)*4800);
}
...
//       case 'v':
//        {  
//                    attachIntteruptArg(v,hallisr, ,CHANGE);
//                    p->intfactor = 2;
//                    break;
//        }
//        case 'w':
//        {
//                    attachIntteruptArg(w,hallisr, ,CHANGE);
//                    p->intfactor = 2;
//                    break;
//        }
//        case 'A':
//        {
//            attachIntteruptArg(u,hallisr, ,CHANGE);
//            attachIntteruptArg(v,hallisr, ,CHANGE);
//            attachIntteruptArg(w,hallisr, ,CHANGE);
//            p->intfactor = 6;
//            break;
...
uint8 isinttring(struct BLDC_4_00 *p){
    return(p->inttrig);
}
...
uint8 nextstate(struct BLDC_4_00 *p, uint8 s, uint8 dir){
  uint8_t *seq = dir?p->seqCW:p->seqCCW;
  return seq[s];
}
